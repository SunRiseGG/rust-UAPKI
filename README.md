...
classes directly through a thin C++ shim.
...
where `library/uapki/src/api/verify-direct.cpp` holds our shim. UAPKI's own
CMake compiles the shim into `libuapki.a` via `aux_source_directory`.
...
and links them directly together with `curl` and the C++ runtime — no runtime
`.so`, `dlopen`, or `rpath`. Requires: `git`, `cmake`, a C++ toolchain, and the
...
// detached, data hashed from a file (for large files)
